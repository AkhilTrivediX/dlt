...
TIncrementalMode = Literal["interval", "pipeline"]
...
class TRuntimeEntryPoint(TEntryPoint):
    ...
    incremental_mode: NotRequired[TIncrementalMode]
...
def resolve_incremental_mode(d: Mapping[str, Any]) -> TIncrementalMode:
    ...
    mode: Optional[TIncrementalMode] = d.get("incremental_mode")
    if mode is not None:
        return mode
