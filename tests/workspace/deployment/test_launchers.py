...
def test_job_launcher_config_driven_incremental_mode() -> None:
    """`[jobs] incremental_mode` from config makes incrementals join the interval."""
...
def test_job_launcher_config_driven_auto_refresh() -> None:
    """`[jobs] auto_refresh_pipeline_mode` from config applies on refresh runs."""
    os.environ["JOBS__AUTO_REFRESH_PIPELINE_MODE"] = "drop_data"
    ep = _entry(f"{WORKSPACE}.batch_jobs", "auto_refresh_pipeline")
    ep["refresh"] = True
    result = job_run(ep, run_id="cfg-apr-1", trigger="manual:")
    assert result == "refresh=drop_data"

    # explicit entry point setting wins over config
    ep = _entry(f"{WORKSPACE}.batch_jobs", "auto_refresh_pipeline")
    ep["refresh"] = True
    ep["auto_refresh_pipeline_mode"] = "drop_sources"
    result = job_run(ep, run_id="cfg-apr-2", trigger="manual:")
    assert result == "refresh=drop_sources"
...
@pytest.mark.parametrize(
    "refresh_signal,expected",
    [(True, "refresh=drop_data"), (False, "refresh=None")],
    ids=["refresh-run", "regular-run"],
)
def test_isolated_job_launcher_config_toml_auto_refresh(
    refresh_signal: bool, expected: str
) -> None:
    """End to end: `[jobs] auto_refresh_pipeline_mode` in workspace config.toml applies
    to pipelines created in the job on refresh runs."""
